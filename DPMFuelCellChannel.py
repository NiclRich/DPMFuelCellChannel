...
condesnation = 0            # Condensation Algorithm (IN PROGRESS) TODO: Does it need to be included?
film_formation = 1          # Film Formation Regime (1 for on)
plug_formation = 1          # Plug Formation Regime (1 for on)
ct = 0                      # Iteration counter
saveresults = 0             # Turn on to save results in .csv and .vtk of final time step
...
D[:, 25] = hlim
Doring[:, 25] = hlim # height limit for each droplet based on the external forces
...
timer_tic_coal = 0
courant = 0.1
visualiseDropletCoutner = 0


## Start time Intergration
for tt in range(100000000000): # maximum time loop
    ...
    adapt_t = np.nan_to_num(adapt_t, pos_inf = dtorig)
    dt = np.min(adapt_t)
    
    # NR: What does this condition?
    if np.max(D[:, 23] / D[:, 24]) > 0.999: 
        dt2 = 1e-3
    ...
    if timer_tic == 1:
        if D.shape[0] > 1:
            for i in np.arange(0, D.shape[0]):
                ...
                distance_compare = D[i, 6] + (poresize) > np.sqrt((D[i, 4] - zp[:, 0])**2 + (D[i, 2] - xp[:, 0])**2 + (D[i,3] - yp[:, 0])**2)
    ...
    # class 1 - emerging
    D[(D[:,0] > 0) & D[:, 5] == 0, 5] = 1               # finds all droplets with volume and assigns as amerging (1)
    D[D[:,5] == 2 & D[:, 24] < theta_GDL, 5] = 1
    # D[D[:,5] == 1, 6] = Rpmax

    # class 2 - isolated
    D[D[:, 5] == 1 & D[:, 24] > theta_GDL, 6] = 2       # finds all emerging droplets and assigns isolated if constant angle regime (2)
    ...
    if coalescence == 1:
        ...
        del D_emerge, pore_cover, dr2, s2
        ...
        if D3.size != 0:
            ...
            thteta_c_total = theta_wall + alpha_GDL
        ...
        DPplug = DPplug.nan_to_num(nan = 0.0, posinf = 0.0, neginf = 0.0)
        ...
        DP = DP.nan_to_num(nan=0.0, posinf=0.0, neginf=0.0)
        DP = np.abs(DP)
        ...
        S[ic, 0] = t
        S[ic, 1] = TotalP
        S[ic, 2] = np.sum(V_evap)
        S[ic, 3] = np.mean(D[D[:, 0] > 0, 19]) # index shift right?

        # Store values in Time matrix
        Time[ic, 0] = toc()
        Time[ic, 1] = D.shape[0]
        Time[ic, 2] = np.sum(D[:, 0]) / (L * W * H)
        Time[ic, 3] = np.sum(D[:, 16]) / (L * W)

        # Save condensation data
        Water[ic, 0] = water_cond
        ...
        if ct > write_precision:
            ...
            segmentRegime.nan_to_num(copy=False)
